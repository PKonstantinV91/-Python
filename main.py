...
with psycopg2.connect(database="clients_db", user="postgres", password="1234") as conn:
    with conn.cursor() as cur:
        ...
        create_db(cur)
        add_client(cur, 'Петр', 'Петров', 'petr@ml.ru', '+79012345678')
        add_phone(cur, '1', '+79876543210')
        change_client(cur, 1)
        delete_phone(cur, '+78889999999')
        # find_client(cur, first_name='Петр')
        client = search_client(cur, first_name='Петр', phones='+79012345678')
        print(client)
        conn.commit()
